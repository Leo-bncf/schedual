...
`;
    ...
    const optaPlannerPayload = {
      ...
      teachers: finalTeachers.map(t => ({
        ...
        max_periods_per_week: t.maxPeriodsPerWeek,
      ...
      subjects: subjects.filter(s => s.is_active && subjectRequirements.some(req => req.subject === (s.code || s.name))).map(s => ({
          ...
          code: s.code || s.name,
          name: s.name,
          hl_hours_per_week: s.hoursPerWeekHL || 5,
          sl_hours_per_week: s.hoursPerWeekSL || 3
      })),
      teachingGroups: teachingGroupsPayload.map(tg => ({
        ...
        sectionId: tg.section_id,
        student_group: tg.student_group,
        studentGroup: tg.student_group,
        subject_id: tg.subject_id,
        subjectId: tg.subject_id,
        level: tg.level,
        required_minutes_per_week: tg.required_minutes_per_week,
        requiredMinutesPerWeek: tg.required_minutes_per_week
      })),
      subjectRequirements: subjectRequirements.map(req => ({
        studentGroup: req.studentGroup,
        student_group: req.studentGroup,
        teachingGroupId: req.teachingGroupId,
        teaching_group_id: req.teachingGroupId,
        sectionId: req.sectionId,
        section_id: req.sectionId,
        subject: req.subject,
        minutes_per_week: req.minutes_per_week,
        minutesPerWeek: req.minutes_per_week
      })),
      lessons: safeLessons.length > 0 ? safeLessons.map(l => ({
        ...
        subject: l.subject,
        studentGroup: l.studentGroup,
        student_group: l.studentGroup,
        teachingGroupId: l.teachingGroupId,
        teaching_group_id: l.teachingGroupId,
        sectionId: l.sectionId,
        ...
        level: l.level,
        yearGroup: l.yearGroup,
        year_group: l.yearGroup,
        studentIds: l.studentIds,
        student_ids: l.studentIds,
        requiredCapacity: l.requiredCapacity,
        required_capacity: l.requiredCapacity,
        blockId: l.blockId,
        block_id: l.blockId,
        teacherId: l.teacherId ? parseInt(l.teacherId, 10) : null,
        teacher_id: l.teacherId ? parseInt(l.teacherId, 10) : null,
        timeslotId: l.timeslotId ? parseInt(l.timeslotId, 10) : null,
        timeslot_id: l.timeslotId ? parseInt(l.timeslotId, 10) : null,
        roomId: l.roomId ? parseInt(l.roomId, 10) : null,
        room_id: l.roomId ? parseInt(l.roomId, 10) : null
      })) : [{
        id: 1001,
        lesson_id: 1001,
        subject: "DUMMY",
        ...
        teacher_id: parseInt(finalTeachers[0].id, 10),
        timeslotId: null,
        timeslot_id: null,
        roomId: null,
        room_id: null
